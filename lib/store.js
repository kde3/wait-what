...
export function wordMatches(word, guess) {
  ...
  return ['ko', 'en', 'ja', 'zh'].some((k) => normalizeText(word[k]) === g);
}
...
export function promptViolation(room, prompt, keyword) {
  ...
  if (keyword) {
    for (const k of ['ko', 'en', 'ja', 'zh']) {
      const v = normalizeText(keyword[k]);
      if (v && p.includes(v)) return keyword[k];
    }
  }
...
export function createRoom(nickname, { name, isPublic, lang } = {}) {
  pruneRooms();
  let code;
  do {
    code = randomCode();
  ...
  const room = {
    ...
    isPublic: !!isPublic,
    lang: ['ko', 'en', 'ja', 'zh'].includes(lang) ? lang : 'ko',
...
export function configRoom(room, playerId, patch = {}) {
  ...
  if (patch.lang !== undefined && ['ko', 'en', 'ja', 'zh'].includes(patch.lang)) room.lang = patch.lang;
...
// 제출: 모드/단계별 확정 동작
export function submitAction(room, playerId, { text } = {}) {
  if (room.status !== 'playing') return { error: 'errNotPlaying' };
  const g = room.game;
  const player = room.players.find((p) => p.id === playerId);
  if (!player) return { error: 'errNotPlayer' };
