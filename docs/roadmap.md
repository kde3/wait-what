...
- **임포스터 흐름**: `turns`(차례대로 그림 생성) → `vote`(임포스터 투표) → 지목 실패면 임포스터 승리로 종료, 지목 성공이면 `guess`(임포스터가 키워드 추리)로 넘어간다. 최다 득표자가 지목되고 동률이면 아무도 지목되지 않는다. 투표는 `g.order` 인덱스로 보내므로 플레이어 id가 노출되지 않는다.
